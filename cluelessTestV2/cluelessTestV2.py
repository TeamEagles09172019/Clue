...
def main():
	...
	turnCount = 0
	 ...
	 # The main game loop. This loop runs one time
    while True: # main game loop
        # Run the player turn to start game
        result = runLevel(turns, turnCount)

        if result in ('solved', 'next'):
            # Go to the next turn.
            turnCount += 1
            if turnCount >= len(turns):
                # If there are no more turns, go back to the first one.
                turnCount = 0
        elif result == 'back':
            # Go to the previous turn.
            turnCount -= 1
            if turnCount < 0:
                # If there are no previous turns, go to the last one.
                turnCount = len(turns)-1
        elif result == 'reset':
            pass # Do nothing. Loop re-calls runLevel() to reset the turn
...
def decorateMap(mapObj, startxy):
    ...
    * """
    ...
    # This needs to be random place for two players
...
def startGame():
    ...
    # Create game objects and starting game state objects. Should be set for
	# multiple players
